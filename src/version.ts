...
export const VERSION_HISTORY: VersionEntry[] = [
  ...
  },
  {
    version: "2.38.19",
    date: "2026-08-27",
    title: "Fix Timezone Offset Badge Alignment and Card Divider Overflow",
    changes: [
  ...
  },
  {
    version: "2.38.18",
    date: "2026-08-27",
    title: "Restore Server Runtime Integrity and Dev Server Startup",
    changes: [
  ...
  },
  {
    version: "2.38.17",
    date: "2026-08-26",
    title: "Fix LDAP Configuration Persistence, Discovery and Status Preservation on Save",
    changes: [
      "Resolved Save & Status Bug: Fixed an issue where the save API endpoint re-read LDAP settings from `homeserver.yaml` instead of `/etc/matrix-synapse/conf.d/ldap.yaml`, causing the response payload to return empty LDAP data and unintentionally clear form fields and disable the status toggle.",
  ...
  },
  {
    version: "2.38.16",
    date: "2026-08-26",
    title: "Dedicated /etc/matrix-synapse/conf.d/ldap.yaml Target Path with Recursive Conflict Comment-Out",
    changes: [
      "Target Path Consolidation: Configured Active Directory / LDAP settings to read from and save exclusively to `/etc/matrix-synapse/conf.d/ldap.yaml`.",
      "Recursive Subfolder & homeserver.yaml Conflict Cleanup: Implemented recursive search across `/etc/matrix-synapse/` and all its subdirectories (including `homeserver.yaml` and `conf.d/`), automatically commenting out duplicate or conflicting LDAP module entries and password_providers so all active LDAP data resides exclusively in `/etc/matrix-synapse/conf.d/ldap.yaml`.",
      "Installer & Stack Script Alignment: Updated `install-matrix-stack.sh` and `matrix-installer.sh` to enforce `/etc/matrix-synapse/conf.d/ldap.yaml` with automated directory creation and chmod 640 permissions.",
      "UI Guidance: Updated LDAP destination badge in ConfigForms to `/etc/matrix-synapse/conf.d/ldap.yaml`."
    ]
  },
  {
    version: "2.38.15",
    date: "2026-08-26",
    title: "Explicit /synapse/conf.d/ldap.yaml Creation and Installer Synchronization",
    changes: [
      "Explicit Target Path Enforcement: Ensured that saving LDAP and Active Directory configurations strictly generates the file at `/synapse/conf.d/ldap.yaml` on target servers.",
      "Installer Synchronization: Updated both `install-matrix-stack.sh` and `matrix-installer.sh` to ensure `/synapse/conf.d/ldap.yaml` is explicitly written, created with proper directories, and given strict permissions (640).",
      "Database and Sandbox State Uniformity: Synchronized local database and sandbox fallback structures to reference `/synapse/conf.d/ldap.yaml` as the canonical path."
    ]
  },
  {
    version: "2.38.14",
    date: "2026-08-26",
    title: "LDAP Single-Source Configuration in /synapse/conf.d/ldap.yaml with Automated Conflict Comment-Out",
    changes: [
      "Target Path Consolidation: Configured Active Directory / LDAP settings to save exclusively and directly to `/synapse/conf.d/ldap.yaml` (with backward-compatible sync to `/etc/matrix-synapse/conf.d/ldap.yaml`).",
      "Automated Conflict Comment-Out: Implemented smart non-destructive scanning across `homeserver.yaml` and all files in `/synapse/conf.d/` and `/etc/matrix-synapse/conf.d/` to comment out duplicate or legacy LDAP modules, password_providers, and `ldap_config` blocks, preventing Synapse authentication conflicts while safely preserving history.",
  ...
  },
  {
    version: "2.38.13",
    date: "2026-08-26",
    title: "SSH-Based Remote PostgreSQL Auto-Detection & Connection Workflow Optimization",
    changes: [
      "Detect DB Info Feature: Added one-click SSH inspection in Connection Manager to automatically connect to the target server (co-located Synapse & PostgreSQL) and parse database credentials (Host, Port, Database Name, Username, Password) directly from homeserver.yaml and conf.d files.",
      "Remote PostgreSQL Auto-Populate: Automatically fills Remote PostgreSQL Parameters with detected credentials and unfolds Advanced Settings upon successful discovery.",
      "Cleaned Connection Creation Workflow: Removed automatic Network Listener modification on connection creation/update to prioritize secure, non-intrusive connection management.",
      "Multi-Language Localization: Complete translation coverage for DB Detection across English, Persian (فارسی), Spanish, Arabic, German, and Russian."
    ]
  },
  {
    version: "2.38.12",
    date: "2026-08-26",
    title: "Connection Manager Auto-Sync to Remote Network Listener & Panel IP Integration",
    changes: [
      "Auto-Add Panel IP to Network Listener: Integrated an toggle/checkbox option in Connection Manager ('Add & Edit Connection') that automatically binds the Raven Panel's IP address to the target server's Matrix Synapse listener bind_addresses (homeserver.yaml) on port 8008.",
      "Two-Way Network Listener Synchronization: Remote homeserver configurations automatically reflect the synced panel IP under the Custom IP Address list in the Network Listener Settings tab with one-click selection.",
      "Dynamic Panel IP Resolution: Added automatic network interface inspection and fallback detection endpoint (`/api/connections/panel-ip`) with custom override capabilities for flexible deployment topologies.",
      "Connection Profile Health & Listener Sync Feedback: Enhanced profile cards and connection test flows to verify both listener binding and Matrix API reachability seamlessly."
    ]
  },
  {
    version: "2.38.11",
    date: "2026-08-25",
    title: "Non-Interactive Installer Hang Fix, Generic Placeholder Sanitization & Clean Wizard State",
    changes: [
  ...
  },
  {
    version: "2.38.10",
    date: "2026-08-24",
    title: "Dedicated Conf.d/ldap.yaml Installer Integration & Strict Element Web Domain Isolation",
    changes: [
  ...
  },
  {
    version: "2.38.9",
    date: "2026-08-24",
    title: "Dedicated Element Web Domain Routing & Wizard Parameter Isolation",
    changes: [
  ...
  },
  {
    version: "2.38.8",
    date: "2026-08-24",
    title: "Direct Synapse Conf.d Architecture for LDAP & Elimination of Intermediate State Conflict",
    changes: [
  ...
  },
  {
    version: "2.38.7",
    date: "2026-08-24",
    title: "Strict Server-State Reflection & Resilient LDAP File Deletion Handling",
    changes: [
  ...
  },
  {
    version: "2.38.6",
    date: "2026-08-24",
    title: "Interactive Server Config Scan & Real-Time Source Detection UI for LDAP",
    changes: [
  ...
  },
  {
    version: "2.38.5",
    date: "2026-08-24",
    title: "Universal Content-Aware LDAP Discovery & Multi-File Deduplication Migration",
    changes: [
  ...
  },
  {
    version: "2.38.4",
    date: "2026-08-24",
    title: "LDAP Boolean YAML Formatting & Attribute Dropdowns with Smart Defaults",
    changes: [
  ...
  },
  {
    version: "2.38.3",
    date: "2026-08-24",
    title: "Explicit Double-Quote Formatting for LDAP Yaml Attributes",
    changes: [
  ...
  },
  {
    version: "2.38.2",
    date: "2026-08-24",
    title: "LDAP Canonical Path Filter Optimization, Package Auto-Install & Homeserver Duplicate Cleanup",
    changes: [
  ...
  },
  {
    version: "2.38.1",
    date: "2026-08-24",
    title: "Fix Config Save Crash & Interactive Active Directory Group Path Filter Builder",
    changes: [
      "Fixed LISTEN_MODE Undefined Crash: Guarded LISTEN_MODE evaluation inside updateHomeserverYamlConfig when updating LDAP-only configurations without homeserver server params, eliminating the 'Cannot read properties of undefined (reading LISTEN_MODE)' error.",
      "Active Directory Object Path & Group Filter Builder: Introduced an intuitive helper allowing administrators to simply input the AD Group Name (e.g. Allow-Chat) and paste the Canonical Name of Object copied from the Active Directory Users & Computers (ADUC) Object Tab.",
      "Dynamic LDAP Filter Generator & Search Base Integration: Automatically reverses OU hierarchies and resolves DC components from the Search Base DN (or path domain) into the standard Synapse memberOf filter query with real-time live preview and 1-click copy.",
      "Step-by-Step Active Directory Guide: Added visual and bilingual instructions guiding admins to open ADUC, select Group Properties, navigate to the Object tab, and copy the Canonical Name of Object."
    ]
  },
  {
    version: "2.38.0",
    date: "2026-08-24",
    title: "Dedicated /etc/matrix-synapse/conf.d/ldap.yaml Config & AD Filter & Directory Search Support",
    changes: [
      "Dedicated Remote LDAP Config File: Migrated LDAP & Active Directory configuration to a clean standalone YAML file at /etc/matrix-synapse/conf.d/ldap.yaml, automatically commenting out legacy or conflicting LDAP module entries in homeserver.yaml to eliminate duplicates.",
      "Full Active Directory Module Structure: Emits standard Synapse ldap_auth_provider.LdapAuthProviderModule specification with enabled, mode, uri, start_tls, base, attributes (uid, name, mail), bind_dn, bind_password, filter, and directory_search (enabled, max_results, base).",
      "Interactive LDAP Filter & AD Group Restriction: Added dedicated UI fields, helper documentation, and 1-click insertion for complex Active Directory security group filter queries (e.g. memberOf).",
      "Directory Search UI Controls: Added controls for user directory discovery search with custom max results and search base."
    ]
  },
  {
    version: "2.37.11",
    date: "2026-08-23",
    title: "Granular Linux Service Reload & Restart Operations for All Stack Services",
    changes: [
      "Dedicated Reload & Restart Action Controls: Enabled instant Reload (systemctl reload) and Restart (systemctl restart) buttons for Matrix Synapse (:8008), Element Web client (:443), PostgreSQL Database (:5432), coturn TURN Server (:3478), Redis Worker Queue, Nginx Web Proxy (:80), fail2ban Brute Protection, and Prometheus Monitoring (:9090).",
  ...
  },
  {
    version: "2.37.10",
    date: "2026-08-23",
    title: "Linux Service Reload Action & Multilingual Syntax Integrity",
    changes: [
      "Linux Service Reload Feature: Added a dedicated Reload button alongside Start, Stop, and Restart in the Linux Service Statuses card for instant systemctl reload on Matrix Synapse, Element, Nginx, Coturn, and PostgreSQL.",
      "Translations Dictionary Syntax Repair: Resolved corrupted Russian and German translation objects in App.tsx to ensure clean compilation across all supported panel languages."
    ]
  },
  {
    version: "2.37.9",
    date: "2026-08-23",
    title: "Homeserver Restart Health Synchronization & Nginx SSL Certificate Preservation",
    changes: [
  ...
  },
  {
    version: "2.37.8",
    date: "2026-08-22",
    title: "Synapse Server Notice Broadcast Delivery & Auth Token Error Handling Fix",
    changes: [
      "Synapse Admin Token Guard: Fixed an issue where Synapse M_FORBIDDEN 'Server notices are not enabled' response was incorrectly treated as an expired auth token, preventing infinite token invalidation loops.",
      ...
      "Target User MXID Normalization & PostgreSQL Fallback: Enhanced recipient resolution to ensure fully-qualified @user:domain format and PostgreSQL user table fallback.",
  ...
  },
  {
    version: "2.37.7",
    date: "2026-08-22",
    title: "Synapse Authoritative Media Repository Upload & Matrix MSC3245/MSC1767 Voice & Media Streaming",
    changes: [
      "Synapse-Authoritative Media Upload: Overhauled uploadMediaToSynapseRepo with SFTP / chunked streaming and direct Synapse local endpoint uploads, preventing Linux argument length limits (ARG_MAX) and obtaining authoritative homeserver content_uri (mxc://...).",
      "Matrix Voice Notes & Media Compatibility: Implemented standard MSC3245 voice flags, MSC1767 waveform and duration audio payloads, and precise MIME detection (audio/ogg, audio/webm, image/*) so Matrix clients (Element) render photos inline and play voice messages flawlessly.",
      "Universal Media Proxy Routes: Added standard Matrix client download & thumbnail endpoints (/_matrix/media/v3/download, /_matrix/client/v1/media/download) with proper CORS, Content-Type, Content-Length, and inline headers to serve attachments without 404 or playback errors."
    ]
  },
  {
    version: "2.37.6",
    date: "2026-08-22",
    title: "Synapse Server Notices Configuration Validation & System Sender Recommendation",
    changes: [
      "Server Notices Configuration Check: Clicking 'Send Notice' / 'Send Notice to (X)' in Matrix User Management now validates whether /etc/matrix-synapse/conf.d/server_notices.yaml exists and is configured on Synapse before attempting broadcast.",
      "Informational Notice Modal: If server_notices.yaml is missing or unconfigured, displays a Light & Dark theme harmonized modal informing the admin in panel language to configure Server Notices first, with a direct button navigating to Synapse Server Notices Configuration.",
      "System Sender Localpart Recommendation: Added clear recommendation notice under 'System Sender Localpart' across both Matrix Admin and Control Hub forms advising administrators to keep the default value 'server'."
    ]
  },
  {
    version: "2.37.5",
    date: "2026-08-22",
    title: "Dedicated Single-Location File Target for Synapse Server Notices (/etc/matrix-synapse/conf.d/server_notices.yaml)",
    changes: [
      "Strict Single Target Path: Removed extraneous target directories and paths (/etc/synapse, config.d, 00_server_notices.yaml) when saving Synapse server notices configuration.",
      "Clean Destination Enforcement: Saving server notices in Synapse Server Notices Configuration now exclusively generates and updates /etc/matrix-synapse/conf.d/server_notices.yaml on the target server without creating duplicate or extra files/directories.",
      "Optimized Config Resolution: Synapse Server Notices retrieval now directly inspects /etc/matrix-synapse/conf.d/server_notices.yaml with clean homeserver.yaml fallback."
    ]
  },
  {
    version: "2.37.4",
