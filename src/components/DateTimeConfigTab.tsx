    ...
    <div className="space-y-6" id="datetime-config-tab">
      ...
      <div className="spatial-glass rounded-3xl p-6 border border-cyan-500/20 relative overflow-hidden bg-gradient-to-br from-cyan-950/20 via-black/40 to-slate-900/30">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
          {/* Time & Clock */}
          <div className="space-y-1 min-w-0 pr-0 md:pr-4 rtl:md:pr-0 rtl:md:pl-4">
            ...
            <p className="text-[11px] text-slate-400 font-mono truncate" title={serverInfo?.formatted || ''}>
              {serverInfo?.formatted || (serverInfo?.timestamp ? new Date(serverInfo.timestamp).toUTCString() : 'Syncing...')}
            </p>
          </div>

          {/* Date & Calendar */}
          <div className="space-y-1 border-y md:border-y-0 md:border-x border-white/10 py-4 md:py-0 md:px-6 min-w-0">
          ...
          {/* Timezone & NTP status */}
          <div className="space-y-2 min-w-0 pl-0 md:pl-2 rtl:md:pl-0 rtl:md:pr-2">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs text-slate-400 shrink-0">{loc('منطقه زمانی فعال:', 'Active Timezone:')}</span>
              <span className="text-xs font-mono font-bold text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-500/20 truncate max-w-[140px]" title={serverInfo?.timezone || 'UTC'}>
                {serverInfo?.timezone || 'UTC'}
              </span>
            </div>

            <div className="flex items-center justify-between gap-2">
              <span className="text-xs text-slate-400 shrink-0">{loc('همگام‌سازی خودکار (NTP):', 'NTP Sync Status:')}</span>
            ...
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs text-slate-400 shrink-0">{loc('میزبان هدف:', 'Target Node:')}</span>
              <span className="text-xs font-mono text-slate-300 truncate max-w-[140px]" title={serverInfo?.serverName || ''}>
