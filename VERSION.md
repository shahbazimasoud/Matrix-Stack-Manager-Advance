...
- **Fix Timezone Offset Badge Alignment and Card Divider Overflow**:
  - **Clock & Timezone Layout Alignment**: Adjusted the Server Date, Time & Timezone digital clock card to wrap time elements gracefully and prevent the timezone offset badge (`+03:30` / `UTC`) from overlapping the column separator border in both RTL and LTR layouts.
  - **Responsive Typography & Padding**: Optimized font sizing, tracking, and directional padding for multi-column digital clock widgets across standard and dense viewport breakpoints.

#### **v2.38.18** - 2026-08-27
- **Restore Server Runtime Integrity and Dev Server Startup**:
  - **Dev Server Restoration**: Restored truncated server runtime blocks and cleanly rebuilt the application bundle.
  - **Module Cleanliness**: Verified complete JSX hierarchy and clean TypeScript build across all components and server endpoints.

#### **v2.38.17** - 2026-08-26
- **Fix LDAP Configuration Persistence, Discovery and Status Preservation on Save**:
  - **Resolved Save & Status Clearing Issue**: Fixed an issue where the save API endpoint re-read LDAP settings from `homeserver.yaml` instead of `/etc/matrix-synapse/conf.d/ldap.yaml`, causing the response payload to return empty LDAP data and unintentionally clear form fields and disable the status toggle.
  - **Strict Protection for ldap.yaml**: Ensured `cleanupLdapFromAllConfigs` completely excludes and protects all `/etc/matrix-synapse/conf.d/ldap.yaml` and `*ldap.yaml` files, commenting out legacy LDAP configurations only in other YAML files.
  - **Preserved Disabled States**: Enhanced `generateLdapYaml` and `parseLdapFromYaml` to persist all configured URI, Base DN, Bind DN, Filter, and attribute settings even when disabled.
  - **Immediate UI State Sync**: Updated frontend configuration state management to instantly reflect the saved LDAP settings without clearing or resetting inputs.

#### **v2.38.16** - 2026-08-26
- **Dedicated /etc/matrix-synapse/conf.d/ldap.yaml Target Path with Recursive Conflict Comment-Out**:
  - **Target Path Consolidation**: Configured Active Directory / LDAP settings when submitted or read to interface exclusively and directly with `/etc/matrix-synapse/conf.d/ldap.yaml`.
  - **Recursive Subfolder & homeserver.yaml Conflict Cleanup**: Implemented recursive scanning across `/etc/matrix-synapse/` and all its subfolders (including `homeserver.yaml`, `conf.d/`, etc.), automatically commenting out duplicate or legacy LDAP modules, `password_providers`, and `ldap_config` blocks with informative banners (`# [Commented out by Raven Panel - Moved to /etc/matrix-synapse/conf.d/ldap.yaml]`), ensuring that active LDAP configurations are centralized strictly in `/etc/matrix-synapse/conf.d/ldap.yaml`.
  - **Installer Synchronization**: Updated `install-matrix-stack.sh` and `matrix-installer.sh` to enforce `/etc/matrix-synapse/conf.d/ldap.yaml` with automated directory creation and chmod 640 permissions.
  - **UI Guidance & Feedback**: Updated LDAP configuration badges and indicators in ConfigForms to display `/etc/matrix-synapse/conf.d/ldap.yaml`.

#### **v2.38.15** - 2026-08-26
- **Explicit /synapse/conf.d/ldap.yaml Target Path Enforcement & Full Stack Installer Sync**:
  - **Explicit Target Path Enforcement**: Ensured that saving LDAP and Active Directory configurations strictly generates and writes to `/synapse/conf.d/ldap.yaml` on target servers with directory auto-creation (`mkdir -p /synapse/conf.d`) and strict permission controls.
  - **Installer Synchronization**: Updated stack installers (`install-matrix-stack.sh` and `matrix-installer.sh`) to write directly to `/synapse/conf.d/ldap.yaml` upon initial installation and reconfiguration.
  - **State Uniformity**: Synchronized database and sandbox fallback structures to reference `/synapse/conf.d/ldap.yaml` as the canonical file.

#### **v2.38.14** - 2026-08-26
- **LDAP Single-Source Configuration in /synapse/conf.d/ldap.yaml with Automated Conflict Comment-Out**:
  - **Target Path Consolidation**: Configured Active Directory / LDAP settings when submitted to save exclusively and directly to `/synapse/conf.d/ldap.yaml` (with backward-compatible sync to `/etc/matrix-synapse/conf.d/ldap.yaml`).
  - **Automated Conflict Comment-Out**: Implemented non-destructive scanning across `homeserver.yaml` and all configuration files in `/synapse/conf.d/` and `/etc/matrix-synapse/conf.d/` to automatically comment out duplicate or legacy LDAP modules, `password_providers`, and `ldap_config` blocks with informative banners (`# [Commented out by Raven Panel - Moved to /synapse/conf.d/ldap.yaml]`), ensuring that active LDAP configurations are centralized in a single file without syntax conflicts.
  - **Comprehensive Scanner & Discovery**: Updated server discovery and scanner tools to prioritize `/synapse/conf.d/ldap.yaml` across both remote SSH connections and local/sandbox environments.
  - **UI Guidance & Feedback**: Updated LDAP configuration badges and indicators in ConfigForms to clearly display the active `/synapse/conf.d/ldap.yaml` destination.

#### **v2.38.13** - 2026-08-26
- **SSH-Based Remote PostgreSQL Auto-Detection & Connection Workflow Optimization**:
  - **Detect DB Info Feature**: Added a prominent "Detect DB Info" action in the Connection Manager (`ConnectionManager.tsx`) that leverages the entered SSH credentials to connect to the target server (where Synapse and PostgreSQL are co-located) and automatically extract database parameters (DB Host, DB Port, DB Name, DB Username, and DB Password) from `homeserver.yaml` and `/etc/matrix-synapse/conf.d/*.yaml`.
  - **Remote PostgreSQL Auto-Populate**: Automatically populates the "Remote PostgreSQL Parameters" fields with detected credentials and unfolds the Advanced Settings section with positive feedback.
  - **Non-Intrusive Connection Setup**: Removed the automatic Network Listener modification option from the connection creation/editing flow, preventing unintended remote homeserver listener changes.
  - **Multi-Language Support**: Full localization across English, Persian (فارسی), Spanish, Arabic, German, and Russian with descriptive context explaining that Synapse and PostgreSQL reside on the target server.

#### **v2.38.12** - 2026-08-26
- **Connection Manager Auto-Sync to Remote Network Listener & Panel IP Integration**:
  - **Auto-Add Panel IP to Network Listener**: Added a dedicated option in the "Add / Edit Connection" screen of the Connection Manager (`ConnectionManager.tsx`) to automatically configure and append the Raven Panel's IP address to the target server's Matrix Synapse listener `bind_addresses` (`homeserver.yaml`) on port 8008.
  - **Two-Way Network Listener Synchronization**: Ensured that the added panel IP is registered and directly visible under the "Custom IP Address" section in the Homeserver Network Listener Settings tab (`ConfigForms.tsx`), with a quick-select action button for seamless management and manual adjustments.
  - **Dynamic Panel IP Resolution**: Added `/api/connections/panel-ip` endpoint to discover and resolve the panel's active IP address across network interfaces, with custom IP override support in the connection form.
  - **Connection Profile Health & Listener Sync Feedback**: Updated connection card badges and test flows to display listener synchronization status and verify Matrix API reachability over SSH and direct ports.

#### **v2.38.11** - 2026-08-25
- **Non-Interactive Installer Hang Fix, Generic Placeholder Sanitization & Clean Wizard State**:
  - **Unattended Installer Hang Prevention**: Audited and guarded all interactive `read` prompts across `matrix-installer.sh` and `install-matrix-stack.sh` with `NON_INTERACTIVE` safety checks, preventing background deployment freezes during remote automated execution.
  - **Universal Placeholder Sanitization**: Replaced all organization-specific placeholder references with generic, privacy-compliant examples (`example.com`, `192.168.1.100`, `dc=example,dc=com`) across Active Directory / LDAP forms, Server & Domains tabs, and multi-language wizard dictionaries (EN, FA, ES, AR, DE, RU).
  - **Clean Wizard Placeholder Display**: Resolved pre-filled dummy values in the Server & Domains setup modal (`InstallWizardModal.tsx`), allowing placeholder hints to render naturally on blank input fields with robust fallbacks during final deployment confirmation.

#### **v2.38.10** - 2026-08-24
- **Dedicated Conf.d/ldap.yaml Installer Integration & Strict Element Web Domain Isolation**:
  - **Authoritative LDAP Conf.d File Generation**: Updated stack installer (`write_ldap_yaml`) to write directly to `/etc/matrix-synapse/conf.d/ldap.yaml` with clean permissions and purged legacy `homeserver.yaml` modules.
  - **Post-Install Reactive LDAP Reflection**: Ensured the Active Directory / LDAP management tab immediately and automatically reads, parses, and populates form fields from `/etc/matrix-synapse/conf.d/ldap.yaml` upon installation completion and tab activation.
  - **Fixed Domain Disconnect in Homeserver Configuration**: Corrected Element Client Domain detection to strictly reflect the designated Web Element domain across Nginx configs, `.conf` variables, and active connection profiles without clobbering from homeserver base URLs.
  - **Removed Domain Filter Blanking**: Eliminated legacy placeholder wipes to ensure real-world Active Directory URIs and Base DNs (including `.local` and custom enterprise domains) are fully preserved and displayed.

#### **v2.38.9** - 2026-08-24
- **Dedicated Element Web Domain Routing & Wizard Parameter Isolation**:
  - **Dedicated Domain Isolation**: Fixed wizard and installer parameter precedence to ensure `ELEMENT_DOMAIN` is strictly honored and never overwritten by homeserver defaults or stale configuration files.
  - **Reactive Wizard Synchronization**: Enhanced the Server & Domains wizard step with smart pre-population, ensuring full visibility and customizability for Element Web, Homeserver, and Well-Known domains.
  - **Nginx Virtual Host Routing**: Improved Element Web Nginx virtual host configuration with explicit `try_files` SPA routing, cache-busting headers for `config.json`, and automatic default site deactivation.
  - **Profile Domain Persistence**: Automatically recorded configured homeserver and Element frontend domains into the active server connection profile upon successful deployment.

#### **v2.38.8** - 2026-08-24
- **Direct Synapse Conf.d Architecture for LDAP & Elimination of Intermediate State Conflict**:
  - **Dedicated Synapse Architecture**: Configured Active Directory and LDAP management to interface exclusively and directly with `/etc/matrix-synapse/conf.d/ldap.yaml` as the authoritative single source of truth.
  - **Elimination of Stale Intermediary Files**: Completely removed generation and reading of `/etc/matrix-stack-ldap.conf`, ensuring that manual or panel configuration updates never suffer from synchronization conflicts or override lag.
  - **Clean Live Reflection**: Ensured all LDAP endpoints (`/api/matrix/ldap/status`, `getAdLdapConfig()`, and discovery) read directly from Synapse YAML configuration without falling back to outdated database cache or obsolete config keys.

#### **v2.38.7** - 2026-08-24
- **Strict Server-State Reflection & Resilient LDAP File Deletion Handling**:
  - **Dynamic Remote File Listing**: Replaced static file path presumptions with live filesystem queries (`find /etc/matrix-synapse/ /etc/synapse/`) to guarantee deleted configuration files are never queried or treated as active sources.
  - **Strict Comment-Aware YAML Validation**: Eliminated false-positive substring matches on commented-out lines in `homeserver.yaml` and `conf.d/*.yaml`, requiring genuine structural YAML configuration modules.
  - **Reliable Form Clearing on File Removal**: When LDAP files are deleted from the server, scanning and status checks strictly reset detected sources to null and clear form fields without resurrecting stale panel database records.

#### **v2.38.6** - 2026-08-24
- **Interactive Server Config Scan & Real-Time Source Detection UI for LDAP**:
  - **Interactive Server Scan Button**: Added "Scan Server Files" action in the Active Directory / LDAP tab to trigger real-time content-aware discovery across all remote configuration files on demand.
  - **Dynamic Form Auto-Population & Reset**: Automatically populates all LDAP form inputs and attribute parameters when discovered on server, or clears all form items to empty/disabled if no LDAP configuration exists.
  - **Real-Time Source File Identification**: Explicitly displays the exact file path(s) where LDAP configuration was detected and synchronized with visual badges and dismissible notifications.

#### **v2.38.5** - 2026-08-24
- **Universal Content-Aware LDAP Discovery & Multi-File Deduplication Migration**:
  - **Universal Content-Based Discovery**: Scans `/etc/matrix-synapse/homeserver.yaml` and all files across `/etc/matrix-synapse/conf.d/*.yaml` by inspecting actual content instead of relying solely on hardcoded file names, automatically auto-filling the LDAP configuration if present in any file.
  - **Comprehensive Multi-File Cleanup on Save**: When saving configurations, automatically removes any legacy or duplicate LDAP modules, `password_providers`, and `ldap_config` sections from `homeserver.yaml` and all other `conf.d/*.yaml` files, migrating the active configuration cleanly into `/etc/matrix-synapse/conf.d/ldap.yaml`.
  - **Detected Sources Tracking**: Passes identified source files to the status inspection endpoint for full visibility into configuration discovery.

#### **v2.38.4** - 2026-08-24
- **LDAP Boolean YAML Formatting & Attribute Dropdowns with Smart Defaults**:
  - **Unquoted Booleans in ldap.yaml**: Formatted boolean values (`enabled`, `start_tls`, `directory_search.enabled`) as raw unquoted booleans (`true`/`false`) in `/etc/matrix-synapse/conf.d/ldap.yaml` while keeping all string values cleanly double-quoted.
  - **LDAP Attribute Dropdowns in UI**: Converted UID Attr, Mail Attr, and Name Attr input fields into dropdown selects with predefined standard options and custom fallback support.
  - **Default Attribute Presets**: Set `sAMAccountName` (UID), `mail` (Mail), and `displayName` (Name) as the primary default values in UI state and server YAML parsers.

#### **v2.38.3** - 2026-08-24
- **Explicit Double-Quote Formatting for LDAP Yaml Attributes**:
  - **Strict Double-Quoting for LDAP Configuration**: Enforced double-quoted values for all essential keys in `/etc/matrix-synapse/conf.d/ldap.yaml` (`module`, `enabled`, `uri`, `base`, `uid`, `name`, `mail`, `bind_dn`, `bind_password`, `filter`, and directory_search `base`).
  - **Robust YAML Generator**: Added dedicated `generateLdapYaml` formatter ensuring clean indentation, proper quote escaping, and 100% compatibility with Matrix Synapse LDAP authentication provider.

#### **v2.38.2** - 2026-08-24
- **LDAP Canonical Path Filter Optimization, Package Auto-Install & Homeserver Duplicate Cleanup**:
  - **Simplified AD Filter Input**: Removed the redundant "Allowed AD Security Group Name" input field, allowing administrators to simply paste the complete "Canonical Name of object" copied directly from the AD Object tab.
  - **Smart Canonical Path Parsing**: Automatically extracts the organizational hierarchy (`OU`) and group name (`CN`) from the Canonical Path to generate the Synapse `memberOf` LDAP filter with live preview and 1-click sample insertion.
  - **Live LDAP Package Status Indicator**: Added a real-time "LDAP Package" status card (`INSTALLED` / `NOT INSTALLED`) on the LDAP dashboard with an interactive 1-click install action.
  - **Automatic Package Installation on Save**: The save configuration handler now automatically checks if `matrix-synapse-ldap3` is installed in the target server environment and installs it automatically if missing.
  - **Homeserver.yaml Duplicate Cleanup**: Ensures all legacy or conflicting LDAP modules and `password_providers` are completely removed from `homeserver.yaml` when saving to `/etc/matrix-synapse/conf.d/ldap.yaml`.

#### **v2.38.1** - 2026-08-24
- **Fix Config Save Crash & Interactive Active Directory Group Path Filter Builder**:
  - **Fixed `LISTEN_MODE` Undefined Crash**: Guarded `LISTEN_MODE` evaluation inside `updateHomeserverYamlConfig` when saving LDAP-only configurations without homeserver server params, completely eliminating the runtime crash `Cannot read properties of undefined (reading 'LISTEN_MODE')`.
  - **Active Directory Object Path & Group Filter Builder**: Added a dedicated helper UI allowing administrators to enter the AD Group Name (e.g. `Allow-Chat`) and paste the Canonical Name of Object (e.g. `company.Extension/Technical-Team/Group/Global/Technical-Groups/Admanager-Helpdesk-Access/`) copied directly from the Active Directory Users & Computers (ADUC) **Object** tab.
  - **Dynamic LDAP Filter Generator & Search Base Integration**: Automatically reverses OU hierarchies and resolves DC components from the Search Base DN (or path domain) into the standard Synapse `memberOf` filter query (`filter: "(&(objectClass=user)(memberOf=CN=Allow-Chat,OU=Admanager-Helpdesk-Access,OU=Technical-Groups,OU=Global,OU=Group,OU=Technical-Team,DC=Company,DC=Extension))"`) with real-time live preview and 1-click copy.
  - **Step-by-Step Active Directory Guide**: Added clear, localized instructions guiding administrators to open ADUC, select Group Properties, navigate to the Object tab, and copy the Canonical Name of Object.

#### **v2.38.0** - 2026-08-24
- **Dedicated `/etc/matrix-synapse/conf.d/ldap.yaml` & Active Directory Filter/Directory Search**:
  - **Dedicated Remote LDAP Configuration**: Migrated LDAP & Active Directory configuration to `/etc/matrix-synapse/conf.d/ldap.yaml` on remote Matrix Synapse servers, automatically commenting out legacy or conflicting LDAP module entries in `homeserver.yaml` to eliminate duplicates.
  - **Synapse Module Schema Compliance**: Generated the complete `ldap_auth_provider.LdapAuthProviderModule` structure containing `enabled`, `mode`, `uri`, `start_tls`, `base`, `attributes` (`uid`, `name`, `mail`), `bind_dn`, `bind_password`, `filter`, and `directory_search` (`enabled`, `max_results`, `base`).
  - **Active Directory Search Filter with Examples**: Added user-friendly filter editor with 1-click sample insertion for `memberOf` security group queries (`(&(objectClass=user)(memberOf=CN=Allow-Chat,OU=Admanager-Helpdesk-Access,OU=Technical-Groups,OU=Global,OU=Group,OU=Technical-Team,DC=Company ,DC=Extension))`) and detailed contextual guide.
  - **Directory Search Controls**: Added configuration controls for user discovery `directory_search` (`enabled`, `max_results`, `base`).

#### **v2.37.11** - 2026-08-23
- **Granular Linux Service Reload & Restart Operations for All Stack Services**:
  - **Dedicated Reload & Restart Action Controls**: Enabled instant Reload (`systemctl reload`) and Restart (`systemctl restart`) buttons for Matrix Synapse (`matrix-synapse :8008`), Element Web client (`nginx-element :443`), PostgreSQL Database (`postgresql :5432`), coturn TURN Server (`coturn :3478`), Redis Worker Queue (`redis-server`), Nginx Web Proxy (`nginx :80`), fail2ban Brute Protection (`fail2ban`), and Prometheus Monitoring (`prometheus :9090`).
  - **Robust Fallback & Service Mapping**: Refined backend `systemctl` execution with fallback chain (`reload-or-restart` / `service reload`) and dual service naming compatibility (`nginx-element` / `nginx`, `redis-server`, `postgresql`) across both local and remote SSH/Agent environments.

#### **v2.37.10** - 2026-08-23
- **Linux Service Reload Action & Multilingual Syntax Integrity**:
  - **Linux Service Reload Feature**: Added a dedicated Reload button (`serviceReload`) with loading feedback (`serviceReloading`) alongside Start, Stop, and Restart in the Linux Service Statuses card for instant `systemctl reload` execution across Matrix Synapse, Element Web, Nginx, Coturn, and PostgreSQL services.
  - **Translations Dictionary Syntax Repair**: Resolved corrupted Russian and German translation objects in `App.tsx` ensuring clean compilation and seamless localization across all 6 supported languages.

#### **v2.37.9** - 2026-08-23
- **Homeserver Restart Health Synchronization & Nginx SSL Certificate Preservation**:
  - **Synapse Post-Restart Health Wait**: Added `waitForSynapseHealthy` helper to poll Synapse client endpoints until the Python process is fully initialized and accepting connections on port 8008 before completing configuration saves, preventing temporary `502 Bad Gateway` responses and Element connectivity errors.
  - **Nginx SSL Certificate Protection**: Enhanced `syncNginxSiteConfigsOnServerParamsChange` to never overwrite existing valid SSL certificates (e.g. Let's Encrypt) with fallback self-signed paths when saving limits, rates, and retention policies.
  - **Selective Nginx Parameter Sync**: Only synchronizes `client_max_body_size` and avoids modifying virtual host server blocks when only media/rate limits are changed.
  - **Policy Form Save Streamlining**: Optimized `handleSavePolicies` in `ConfigForms` to execute sequentially without launching concurrent conflicting modals.

#### **v2.37.8** - 2026-08-22
- **Synapse Server Notice Broadcast Delivery & Auth Token Error Handling Fix**:
  - **Synapse Admin Token Guard**: Fixed an issue where Synapse `M_FORBIDDEN` ("Server notices are not enabled on this homeserver") response was incorrectly treated as an expired auth token, preventing authentication retry loops.
  - **Multi-File Configuration Detection**: `getSynapseServerNoticesConfig` now inspects `conf.d/server_notices.yaml`, `homeserver.yaml`, and database configurations gracefully.
  - **Target User MXID Normalization & PostgreSQL Fallback**: Enhanced recipient resolution to ensure fully-qualified `@user:domain` format and PostgreSQL active user table fallback when sending server notices to all users.
  - **Informative Feedback & Toast Notifications**: Detailed error reporting and success/warning toast notifications when sending broadcasts with media or voice messages.

#### **v2.37.7** - 2026-08-22
- **Synapse Authoritative Media Repository Upload & Matrix MSC3245/MSC1767 Voice & Media Streaming**:
  - **Authoritative Synapse Media Upload**: Overhauled `uploadMediaToSynapseRepo` with SFTP / chunked streaming directly to the Synapse media repo via local curl HTTP endpoints, preventing Linux `ARG_MAX` truncation and generating authoritative homeserver `content_uri` (`mxc://...`).
  - **Matrix Voice Notes & Media Compatibility**: Implemented standard MSC3245 voice flags, MSC1767 waveform and duration audio payloads, and precise MIME detection (`audio/ogg`, `audio/webm`, `image/*`) so Matrix clients (Element) render photos inline and play voice messages flawlessly without "Unable to show image due to error" or "Error processing voice message".
  - **Universal Media Proxy Routes**: Added standard Matrix client download & thumbnail endpoints (`/_matrix/media/v3/download`, `/_matrix/client/v1/media/download`) with proper CORS, Content-Type, Content-Length, and inline headers to serve attachments reliably.

#### **v2.37.6** - 2026-08-22
- **Synapse Server Notices Configuration Validation & System Sender Recommendation**:
  - **Pre-flight Configuration Verification**: Clicking `Send Server Notice` or `Send Notice to (X)` in Matrix User Management now validates whether `/etc/matrix-synapse/conf.d/server_notices.yaml` exists and is configured on Synapse before attempting to send messages.
  - **Light/Dark Harmonized Info Modal**: If `server_notices.yaml` is missing or unconfigured, displays an informational modal (compatible with Light & Dark themes) explaining in the panel's active language that Server Notices must be configured first, featuring a direct button to navigate to Synapse Server Notices Configuration.
  - **System Sender Localpart Warning**: Added a prominent recommendation note under `System Sender Localpart` in both Synapse Server Notices Configuration modals, advising administrators to keep the default value `server`.

#### **v2.37.5** - 2026-08-22
- **Dedicated Single-Location File Target for Synapse Server Notices (`/etc/matrix-synapse/conf.d/server_notices.yaml`)**:
  - **Strict Destination Path**: Removed all extra target directories and paths (`/etc/synapse`, `config.d`, `00_server_notices.yaml`) from server notice persistence routines.
  - **Clean Single-File Creation**: Clicking Save in Synapse Server Notices Configuration now exclusively generates and updates `/etc/matrix-synapse/conf.d/server_notices.yaml` on the destination server, preventing redundant directories or duplicate files from being created.
  - **Direct Retrieval**: Synchronized server notices retrieval to prioritize reading directly from `/etc/matrix-synapse/conf.d/server_notices.yaml`.
